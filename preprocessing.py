def sequence_maker(df, sequence_length=8):
    ...
    count = 0
    ID = df.at[2, 'ID'] # Get the Subject ID from the input dataframe
    for i in df.values:
    ...
    df = pd.DataFrame(sequential_data,
                      columns=['mean_x_speed', 'mean_y_speed', 'mean_speed', 'mean_acc', 'mean_jerk', 'mean_ang',
                               'mean_curve','std_x_speed', 'std_y_speed', 'std_speed', 'std_acc', 'std_ang', 'std_jerk',
    ...
    #df.to_csv(f"{ID}_Extracted_Features.csv")
    return df
